...
const (
	AnnotationIAMRoleKey   = "iam.amazonaws.com/role"
	indexPodIP             = "byIP"
	indexPodRole           = "byRole"
	indexPodServiceAccount = "byServiceAccount"
)
...
func NewPodCache(source cache.ListerWatcher, iamRoleCache *IamRoleCache, syncInterval time.Duration, bufferSize int) *PodCache {

	podIndexerCache := PodIndexerCache{
		IamRoleCache: iamRoleCache,
	}

	indexers := cache.Indexers{
		indexPodIP:             podIndexerCache.PodIPIndex,
		indexPodRole:           podIndexerCache.PodRoleIndex,
		indexPodServiceAccount: podIndexerCache.PodServiceAccountIndex,
	}
	pods := make(chan *v1.Pod, bufferSize)
	podHandler := &podHandler{pods, iamRoleCache}
...
// findRoleForPod returns the Pod with the provided IP address
func (s *PodCache) findRoleForPod(_ *v1.Pod) (string, error) {
	return "", nil
}
...
func (s *PodCache) GetRoleForPod(pod *v1.Pod) (string, error) {
	return s.findRoleForPod(pod)
}

func (pci *PodIndexerCache) PodIPIndex(obj interface{}) ([]string, error) {
	pod := obj.(*v1.Pod)
...
func (pci *PodIndexerCache) PodRoleIndex(obj interface{}) ([]string, error) {
	pod := obj.(*v1.Pod)
	role := pci.IamRoleCache.PodRole(pod)
	if role == nil {
		return []string{}, nil
	}

	return []string{role.LookupID()}, nil
}

func (pci *PodIndexerCache) PodServiceAccountIndex(obj interface{}) ([]string, error) {
	pod := obj.(*v1.Pod)
	sa := PodServiceAccount(pod)
	if sa == "" {
		return []string{}, nil
	}
...
func PodServiceAccount(pod *v1.Pod) string {
...
func (o *podHandler) announce(pod *v1.Pod) {
	...
	if o.iamRoleCache.PodRole(pod) == nil {
		return
	}
